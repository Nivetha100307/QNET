...
export const StageCard: React.FC<StageCardProps> = ({ stage, isCurrentActive }) => {
  const stageNumFormatted = stage.number < 10 ? `0${stage.number}` : `${stage.number}`;
  ...
  if (stage.status === 'RUNNING' || isCurrentActive) {
  ...
  return (
    <div className={`p-3 rounded-xl border transition-all duration-300 flex flex-col justify-between h-24 ${borderClass}`}>
      <div className="flex items-center justify-between">
        <span className="text-[10px] font-mono font-bold px-1.5 py-0.5 rounded bg-slate-900 text-cyan-400 border border-slate-800">
          STAGE {stageNumFormatted}
        </span>
        <span className={`text-[9px] font-mono px-2 py-0.5 rounded border flex items-center gap-1 ${badgeClass}`}>
          <Icon className={`w-3 h-3 ${stage.status === 'RUNNING' ? 'animate-spin' : ''}`} />
          {stage.status}
        </span>
      </div>

      <div className="space-y-1 mt-1">
        <div className="text-xs font-bold text-slate-100 truncate" title={stage.name}>
          {stage.name}
        </div>
        <div className="text-[9px] text-slate-400 font-mono flex items-center justify-between">
          <span className="truncate">{stage.category}</span>
